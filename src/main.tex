...
\begin{document}
    ...
    \subsection{刺激量算出部}
    提案システムでは，予めユーザの不快に感じない最大の刺激量 (GVS:$I_{max}$, $U_{max}$) を設定し，VR上の位置情報から算出される刺激量 (GVS: $i$，振動: $u$) を蒸散した刺激量を提示する．ただし，$-1\le i\le 1$, $0\le u\le 1$である．ここで速度，加速度は基準値を1 $m/s$，1 $m/s^2$ として正規化して用いる．
    \begin{equation}
